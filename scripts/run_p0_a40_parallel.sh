...
stage1_config="${1:-configs/stage1_p0_a40.yaml}"
stage2_config="${2:-configs/stage2_p0_a40.yaml}"
experiment_dir="${EXPERIMENT_DIR:-outputs/experiments/p0_a40_20260508}"
folds=(${FOLDS:-0 1 2 3})
...
for fold in "${folds[@]}"; do
  ...
  (
    ...
    echo "fold=${fold} gpu=${gpu} started $(date -Is)"
    ...
    echo "fold=${fold} gpu=${gpu} finished $(date -Is)"
...
if [ "${status}" -ne 0 ]; then
  echo "At least one fold failed; skip pooled OOF postprocess search." >&2
  exit "${status}"
fi
